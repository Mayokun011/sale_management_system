...
public interface CategoryRepository extends JpaRepository<Category, Long> {
    ...
    @Query(value = "select new com.amexcode.salesmanagementsystem.products.dto.CategoryDto" +
            "(c.id, c.name, count(p.category.id)) " +
            "from Category c left join Product p on c.id = p.category.id " +
            "where c.activate = true and c.deleted = false " +
            "group by c.id ")
